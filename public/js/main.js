...
let possibleMoves = [];
let lastMove = null; // { from, to } in algebraic
...
// ─── Render Board ──────────────────────────────────────────────────────────────
const renderBoard = () => {
  const board = chess.board(); // board[0][0] = a8 (top-left for white)
  ...
  rowOrder.forEach((row) => {
    colOrder.forEach((col) => {
      ...
      // Store LOGICAL coordinates so handleMove always works correctly
      ...
      // Highlight selected square & possible moves
      ...
      if (square) {
        ...
        // Draggable only if it's your piece and your turn
        ...
        pieceEl.addEventListener("dragstart", (e) => {
          ...
          // Re-render to show dots without full DOM rebuild
        ...
        pieceEl.addEventListener("dragend", () => {
          ...
          draggedPiece = null;
          // sourceSquare cleared after drop or if drop failed
        });
...
// Opponent's move arrives here (server uses broadcast, so we don't get our own)
socket.on("move", (move) => {
  const result = chess.move(move);
  if (result) {
